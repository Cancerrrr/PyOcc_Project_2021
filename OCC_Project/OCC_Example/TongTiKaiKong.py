...
class TTKaikong():
    def __init__(self, D_i = 1000, t = 35, l = 2000, R_n = 100, L_pr1 = 50, t_n = 50, t_n2 = 0, t_e = 0, L_pr2 = 0, L_pr3 = 0, Hole_pos = 1000):
        ...
        edge = BRepBuilderAPI_MakeEdge(gp_Pnt(r, 0, 0), gp_Pnt(0, 0, 0)).Edge()
        # 筒体
        cylinder1 = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, 0, 0), gp_Dir(0, 1, 0)), R, l).Shape()
        cylinder2 = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, 0, 0), gp_Dir(0, 1, 0)), r, l).Shape()
        new_thing1 = BRepAlgoAPI_Cut(cylinder1, cylinder2).Shape()

        # 布尔剪打孔
        my_cylinder = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, Hole_pos, 0), gp_Dir(1, 0, 0)), R_n + t_n,
                                               l).Shape()  # 参数按照顺序为孔圆心位置，方向，半径，孔深
        new_thing2 = BRepAlgoAPI_Cut(new_thing1, my_cylinder).Shape()

        # 加管
        cylinder3 = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, Hole_pos, 0), gp_Dir(1, 0, 0)), R_n + t_n,
                                             r + t + L_pr1).Shape()
        cylinder4 = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, Hole_pos, 0), gp_Dir(1, 0, 0)), R_n, r + t + L_pr1).Shape()
        new_thing3 = BRepAlgoAPI_Cut(cylinder3, cylinder4).Shape()
        new_thing = BRepAlgoAPI_Fuse(new_thing2, new_thing3).Shape()

        # 切内部管
        Circle = GC_MakeCircle(gp_Ax2(gp_Pnt(0, 0, 0), gp_Dir(0, 1, 0)), r).Value()
        ...
        for shape in TopologyExplorer(splitter.Shape()).faces():
            True
        B = BRepPrimAPI_MakeCylinder(gp_Ax2(gp_Pnt(0, 0, 0), gp_Dir(0, 1, 0)), r, l).Shape()
