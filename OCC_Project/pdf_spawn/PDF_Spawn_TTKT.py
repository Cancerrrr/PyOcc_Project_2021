...
class PDFGenerator:
    def __init__(self, TTKT_Parameter={}):
        ...
        self.basic_data = [['开孔补强计算', '', '计算单位', '压力容器计算软件', '', ''],
                      ...
                      ['壳体材料许用应力', '140', 'MPa', '', '', ''],
                      ['接管轴线与筒体表面法线夹角', '', thita, '', '', ''],
                      ['接管实际外伸长度', L_pr1, 'mm', '接管连接型式', '插入式接管壁安放在容器壁上', ''],
                      ['接管实际内伸长度', L_pr2, 'mm', '接管材料', 'Q245R', ''],
                      ['接管壁公称厚度', t_n, 'mm', '名称及类型', '板材', ''],
                      ['接管内半径', R_n, 'mm', '补强圈材料名称', '', ''],
                      ['接管腐蚀裕量C2t', C_2t, 'mm', '补强圈宽度', W, 'mm'],
                      ['', '', '', '补强圈厚度', t_e, 'mm'],
                      ['接管厚度负偏差C1t', '0', 'mm', '补强圈厚度负偏差C1r', '', 'mm'],
                      ['接管材料许用应力', '140', 'MPa', '补强圈许用应力', '', 'mm'],
                      ['开孔补强计算', '', '', '', '', ''],
                      ['沿容器壁补强范围', L_R, 'mm', '最大局部一次薄膜应力', P_L, 'MPa'],
                      ['伸出在容器外表面沿接管壁补强范围', L_H, 'mm', '平均一次薄膜应力', sigma_avg, 'MPa'],
                      ['伸入在容器内表面沿接管壁补强范围', L_I, 'mm', '总体一次薄膜应力', sigma_eire, 'MPa'],
                      ['壳体有效半径', R_eff, 'mm', '壳体有效厚度', t_eff, 'mm'],
                      ['接管有效外伸长度', L_pr1, 'mm', '接管有效内伸长度', L_pr2, 'mm'],
                      ['接管开口处有效总面积', A_T, 'mm^2', '内压在壳体上引起的力', f_S, 'mm'],
                      ['内压在容器外侧接管上引起的力', f_N, 'N', '内压引起的不连续的力', f_Y, 'mm^2'],
                      ['许用应力', '', 'MPa', '接管最大许用应力', P_MAX, 'mm^2'],
                      ['结论:'+check_P_L, '', '', '']]
